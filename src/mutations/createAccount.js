...
export default async function createAccount(context, input) {
  ...
  if (!anyAccount) {
    ...
    const emailAddresses = emails.map((emailRecord) =>
      emailRecord.address.toLowerCase()
    );
    invites = await AccountInvites.find({
  ...
  if (groups.size > 0) {
    await context.mutations.updateGroupsForAccounts(
